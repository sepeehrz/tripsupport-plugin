...
export default {
  ...
  async mounted() {
    ...
    this.displayOrigin =
      parsedGetLastSearch.origin.ac +
      '-' +
      parsedGetLastSearch.origin.ct +
      '-' +
      parsedGetLastSearch.origin.an;
    this.origin = parsedGetLastSearch.origin;
    this.displayDestination =
      parsedGetLastSearch.destination.ac +
      '-' +
      parsedGetLastSearch.destination.ct +
      '-' +
      parsedGetLastSearch.destination.an;
    this.destination = parsedGetLastSearch.destination;
    if (parsedGetLastSearch.date) {
      this.lastDate = parsedGetLastSearch.date;
      this.departDate = this.changeFormat(parsedGetLastSearch.date.startDate);
    }
  },
  computed: {
    isMobile() {
  ...
  methods: {
    ...
    fillInput() {
      if (this.originItems.length && this.origin.length && !this.origin.ct) {
